#!/usr/bin/env python3
"""Check rule 3 from MySQL.

Rule:
finished inbound quantity = production cost management inbound quantity

finished inbound quantity:
  actual_num + defective_actual_num from finish inbound detail,
  filtered by the finish header billing_time by default

production cost management inbound quantity:
  produce_num from erp_cost.produce_cost
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check rule 3 in MySQL")
    ...
    parser.add_argument(
        "--output",
        default="D:/codex/工厂账务核对/fms-reconciliation/output/rule3_2026_03.csv",
...
def fetch_finish_map(
    ...
    sql = (
        ...
        "SUM(COALESCE(d.actual_num, 0) + COALESCE(d.defective_actual_num, 0)) AS qty "
...
def fetch_produce_map(
    conn,
    produce_table: str,
    calc_month: str,
) -> Dict[str, Decimal]:
    sql = (
        "SELECT material_code, SUM(COALESCE(produce_num, 0)) AS qty "
        f"FROM {produce_table} "
        "WHERE calc_month = %s "
...
def build_rows(
    ...
    produce_map: Dict[str, Decimal],
) -> List[Dict[str, str]]:
    rows: List[Dict[str, str]] = []
    for material_code in sorted(set(finish_map) | set(produce_map)):
        finish_qty = finish_map.get(material_code, Decimal("0"))
        produce_qty = produce_map.get(material_code, Decimal("0"))
        diff = finish_qty - produce_qty
        rows.append(
            {
                "material_code": material_code,
                "finish_qty": str(finish_qty),
                "produce_qty": str(produce_qty),
                "diff": str(diff),
...
def write_csv(path: str, rows: Iterable[Dict[str, str]]) -> None:
    ...
    row_list = list(rows)
    fieldnames = ["material_code", "finish_qty", "produce_qty", "diff", "status"]
...
def main() -> int:
    ...
    rows = build_rows(finish_map, produce_map)
    diff_rows = [row for row in rows if row["status"] != "MATCH"]
    finish_total = sum(finish_map.values(), Decimal("0"))
    produce_total = sum(produce_map.values(), Decimal("0"))
    total_diff = finish_total - produce_total
    ...
    print(f"produce total: {produce_total}")
