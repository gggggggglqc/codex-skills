#!/usr/bin/env python3
"""Check rule 2 from MySQL.

Rule:
finished inbound quantity = cost restoration finished quantity
...
cost restoration finished quantity:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check rule 2 in MySQL")
    ...
    parser.add_argument(
        "--output",
        default="D:/codex/工厂账务核对/fms-reconciliation/output/rule2_2026_03.csv",
...
def build_rows(
    ...
    for material_code in sorted(set(finish_map) | set(cost_map)):
        ...
        cost_qty = cost_map.get(material_code, Decimal("0"))
        diff = finish_qty - cost_qty
        rows.append(
            {
                "material_code": material_code,
                "finish_qty": str(finish_qty),
                "cost_qty": str(cost_qty),
                "diff": str(diff),
...
def write_csv(path: str, rows: Iterable[Dict[str, str]]) -> None:
    ...
    row_list = list(rows)
    fieldnames = ["material_code", "finish_qty", "cost_qty", "diff", "status"]
